...
##   godot --headless --path <โฟลเดอร์โปรเจกต์> res://addons/mugaplex/tests/run_tests.tscn
## หมายเหตุ: path preload ด้านล่างสมมติว่าโฟลเดอร์นี้อยู่ที่
## res://addons/mugaplex/ ตามที่ REGISTRY_PATH อื่นๆ ในโปรเจกต์ใช้อยู่แล้ว
## ถ้าคุณวาง plugin ไว้คนละที่ ให้แก้ path ด้านล่างให้ตรงกับตำแหน่งจริง

const TEST_SUITES: Array[Script] = [
	preload("res://addons/mugaplex/inventory/tests/test_inventory_slot.gd"),
	preload("res://addons/mugaplex/inventory/tests/test_perishable_module.gd"),
	preload("res://addons/mugaplex/inventory/tests/test_inventory_component.gd"),
	preload("res://addons/mugaplex/inventory/tests/test_equipment_component.gd"),
